...
# 3) 차량 접근 노드 — 웹캠이 발행한 차량 위치로 Nav2 goal 전송, 오크디 depth로 전방 정지 판단
...
- `vehicle_approach_node`는 웹캠 인식 노드가 발행하는 `/webcam/vehicle_initial_pose`, 오크디 depth, Nav2, TF(`map`↔`base_link`)가 실제로 배선되어 있어야 정상 동작함. 하드웨어 연결 전이면 에러가 날 수 있음 (`docs/NEXT_STEPS.md` §4 참고).
...
- 로봇 하드웨어(터틀봇) 쪽 토픽/액션이 전부 `/robot11` 네임스페이스 아래에 있어 다음 항목들을 `robot11`로 하드코딩해둠:
  ...
  - `vehicle_approach/config/params.yaml`의 `depth_topic` → `/robot11/oakd/...`
