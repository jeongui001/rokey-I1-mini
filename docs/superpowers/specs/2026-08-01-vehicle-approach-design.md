...
- **정지 판정(W2)**: 연속 N프레임 동안 탐지 중심점의 위치 변화가 픽셀 임계치 이내면 정지로 판단한다. N과 임계치는 TBD(§7).
- **다중 탐지 시 선택 규칙**: ROI 내 탐지가 여러 개면 confidence가 가장 높은 탐지를 사용한다.
- **픽셀→map 변환(W3)**: 사전 캘리브레이션한 픽셀-map 대응점 쌍으로 호모그래피 행렬을 구해 변환한다. 감지 영역은 평면(지면)이라고 가정하며, 변환에 사용하는 픽셀 좌표는 바운딩박스의 하단 중심(지면 접촉점 근사)이어야 정확하다 — 바운딩박스 상단이나 중심을 쓰면 차량 높이만큼 오차가 생긴다. 캘리브레이션 대응점의 구체 개수/배치는 TBD(§7).
